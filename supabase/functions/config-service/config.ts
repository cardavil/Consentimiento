...
export async function handle_get_config(_body: Record<string, unknown>, req: Request): Promise<Response> {
  ...
  const { data: sms } = await ctx.admin
    .from('org_sms_config').select('gateway_url, enabled').eq('organization_id', ctx.org_id).maybeSingle();
  const { data: wa } = await ctx.admin
    .from('org_whatsapp_config').select('phone_number_id, waba_id, display_phone, enabled').eq('organization_id', ctx.org_id).maybeSingle();
...
export async function handle_set_sms(body: Record<string, unknown>, req: Request): Promise<Response> {
  ...
  const { error } = await ctx.admin.from('org_sms_config').upsert(row, { onConflict: 'organization_id' });
...
export async function handle_set_whatsapp(body: Record<string, unknown>, req: Request): Promise<Response> {
  ...
  const { error } = await ctx.admin.from('org_whatsapp_config').upsert(row, { onConflict: 'organization_id' });
...
export async function handle_test(body: Record<string, unknown>, req: Request): Promise<Response> {
  ...
  try {
    if (channel === 'whatsapp') await send_whatsapp_otp(ctx.admin, ctx.org_id, phone, '123456');
    else if (channel === 'sms') await send_sms_otp(ctx.admin, ctx.org_id, phone, '123456');
